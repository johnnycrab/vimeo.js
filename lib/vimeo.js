...
Vimeo.prototype.streamingUpload = function (path, video_uri, callback, progress_callback) {
    ...
    this.request(options, function (err, ticket, status, headers) {
        if (err) {
            return callback(new Error('Requesting ticket err: ' + err.message));
        }
        
        var file = new FileStreamer(path, ticket.upload_link_secure, ticket.ticket_id, progress_callback);
        
        file.ready(function () {
        	_self.deleteTicket(ticket.complete_uri, 0, callback)
...
Vimeo.prototype.deleteTicket = function (complete_uri, num, callback) {
	var _self = this;
	...
	this.request({
    	...
    	if (err) {
    		if (++num > 2) {
    			callback(new Error('Completing upload failed for url ' + ticket.upload_link_secure + ': ' + err.message), body, status, headers);
    		} else {
    			_self.deleteTicket(complete_uri, num, callback);
    		}
    	} else {
    		callback(err, body, status, headers);
    	}
    });
}
