 ...
 */
var FileStreamer = module.exports = function FileStreamer(file_path, upload_endpoint, ticket_id, progress_callback) {
    if (!file_path) {
    ...
    if (this._endpoint.protocol === 'https:' && !this._endpoint.port) {
        this._endpoint.port = 443;
    }
...
FileStreamer.prototype._ticket_id = null;
...
FileStreamer.prototype._readStreamFd = null;
FileStreamer.prototype.progress_callback = null;

// hack by Johnny. We count the numbers of errors and if they are greater than 10, only then will we abort the upload
FileStreamer.prototype._streamUploadMaxErrors = 0;
FileStreamer.prototype._sentBytes = 0;
...
FileStreamer.prototype._ready = function () {
    ...
    this._getNewStart(function (err, start) {
        ...
        if (start >= _self._file_size) {
            ...
            console.log("NEEDED: " + _self._file_size);
...
FileStreamer.prototype._streamChunk = function (start) {
    ...
    _self._sentBytes = start
    _self._putFile(start, function (put_err, code, headers) {

        // Catches a rare vimeo server bug and exits out early
        if (put_err && code && ++_self._streamUploadMaxErrors >= 100) {
            _self._cleanupFds();
            return _self._error(new Error(put_err.message + " streamErrors: 100, successfulBytes: " + _self._sentBytes));
        }
...
FileStreamer.prototype._putFile = function (start, callback) {
    ...
    // cleanup any remaining read stream file descriptor
    this._closeReadStreamFd();
...
FileStreamer.prototype._cleanupFds = function () {
    this._closeFile();
    this._closeReadStreamFd();
};

/**
 * Cleanup the read stream of the file, if there's still one
 */
FileStreamer.prototype._closeReadStreamFd = function () {
    if (this._readStreamFd) {
        fs_module.close(this._readStreamFd, function (e) {});
        this._readStreamFd = null;
    }
};
...
FileStreamer.prototype._closeFile = function () {
    if (this._fd) {
        fs_module.close(this._fd, function (e) {});
        this._fd = null;
    }
};
...
FileStreamer.prototype._getNewStart = function (next) {
    ...
    this._upload_endpoint_request({
        ...
        if (err) {
            return next(new Error('Error when getting new start:' + err.message));
        }

        if (status === 308) {
            return next(null, parseInt(headers.range.split('-')[1] + 1));
        } else {
